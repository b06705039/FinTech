...
def myStrategy(pastPriceVec, currentPrice, windowSize, alpha, beta,rsiL,rsiS):
...
if __name__=='__main__':
				...
				if beta>alpha:
					...
					for rsiL in range(rsiLMin, rsiLMax+1):
						...
						for rsiS in range(rsiSMin, rsiSMax+1):
							print("\trsiS=%d" %(rsiS), end="")
							if rsiL>rsiS:
								returnRate=computeReturnRate(adjClose, windowSize, alpha, beta,rsiL,rsiS)		# Start the whole run with the given parameters
	...
	print("Best settings: windowSize=%d, alpha=%d, beta=%d, rsiL=%d, rsiS=%d ==> returnRate=%f" %(windowSizeBest,alphaBest,betaBest,rsiLBest ,rsiSBest,returnRateBest))		# Print the best result
